...
pub fn locateGraphicsOutput(boot_services: *uefi.tables.BootServices) !GraphicsInfo {
    log.debug(@src(), "locating graphics output protocol", .{});
    const res = boot_services.locateProtocol(uefi.protocol.GraphicsOutput, null) catch |err| {
        log.err(@src(), "locating graphics output protocol failed: {s}", .{@errorName(err)});
        return err;
    };
    const graphics_output = res orelse {
        log.err(@src(), "graphics output protocol not found!", .{});
        return error.NotFound;
    };

    log.debug(@src(), "querying graphics mode info", .{});
    const mode = graphics_output.mode;
    log.info(@src(), "current graphics mode = {}", .{mode.mode});

    var i: u32 = 0;
    while (i < mode.max_mode) : (i += 1) {
        const mode_info = graphics_output.queryMode(i) catch |err| {
            log.err(@src(), "querying graphics mode failed: {s}", .{@errorName(err)});
            return err;
        };

        if (mode.mode == i) {
            log.info(@src(), "  resolution and pixel format: {}x{} {s}", .{
