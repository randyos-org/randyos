//! Linux mmap()/mprotect()/mlock()/madvise() ABI constants for powerpc
//! (32-bit).
//!
...
//!   - `arch/powerpc/include/uapi/asm/mman.h` -- unlike the other three
//!     arches, powerpc's header includes ONLY asm-generic/mman-common.h. It
//!     does **not** include asm-generic/mman.h at all, and instead
//!     independently (re)defines MAP_GROWSDOWN/DENYWRITE/EXECUTABLE, its own
//!     MAP_NORESERVE/MAP_LOCKED/MCL_CURRENT/MCL_FUTURE/MCL_ONFAULT values,
//!     PROT_SAO, a MAP_RENAME alias, and a powerpc-only PKEY_DISABLE_EXECUTE
//!     override. See the per-group notes below for exactly which values
//!     match the generic asm-generic/mman.h values and which genuinely
//!     diverge.
...
//!
//! This is the most divergent of the four per-arch mman files -- powerpc
//! opts out of the generic asm-generic/mman.h chain entirely, so several
//! values below differ from x86_64/arm/aarch64 even though the *names* are
//! shared. Also, because powerpc never includes asm-generic/mman.h, it has
//! no SHADOW_STACK_SET_TOKEN/SET_MARKER equivalent -- those constants are
//! intentionally absent from this file (powerpc has no map_shadow_stack()
//! support upstream at this commit).
//!
...
const log = std.log.scoped(.abi_mman_powerpc);
...
// powerpc-specific: uses the bit the generic header reserves for
// arch-specific use at 0x10 (aarch64 uses that same bit for PROT_BTI
// instead; x86_64/arm leave it undefined).
/// Strong Access Ordering: powerpc-specific hint disabling weak memory
/// ordering/reordering optimizations for this page.
pub const PROT_SAO: u32 = 0x10; // Strong Access Ordering
...
// Legacy SunOS-terminology alias for MAP_ANONYMOUS; powerpc-specific name,
// same value/meaning as MAP_ANONYMOUS.
/// Legacy SunOS-terminology alias for MAP_ANONYMOUS; identical meaning.
pub const MAP_RENAME: u32 = MAP_ANONYMOUS;
// DIVERGES from generic (asm-generic/mman.h has MAP_NORESERVE = 0x4000).
// powerpc defines this independently at a completely different value.
...
// DIVERGES from generic (asm-generic/mman.h has MAP_LOCKED = 0x2000).
// powerpc defines this independently at a completely different value.
...
// powerpc redefines these three independently of asm-generic/mman.h (since
// it never includes that header), but the values happen to MATCH the
// generic ones exactly.
...
// MCL_* -- mlockall() flags.
// ALL THREE DIVERGE from generic (asm-generic/mman.h has
// MCL_CURRENT/FUTURE/ONFAULT = 1/2/4). powerpc defines these independently
// at completely different values, all in the same 0x2000-0x8000 range as
// its own MAP_LOCKED (0x80) neighborhood of bits, not the generic 1/2/4.
/// Lock all pages currently mapped into the process's address space.
pub const MCL_CURRENT: u32 = 0x2000;
/// Lock all pages mapped into the process's address space in the future
/// as well.
pub const MCL_FUTURE: u32 = 0x4000;
/// With MCL_CURRENT/MCL_FUTURE, only lock pages once they're actually
/// faulted in, rather than immediately.
pub const MCL_ONFAULT: u32 = 0x8000;
...
//
// powerpc OVERRIDES part of the generic set: it adds PKEY_DISABLE_EXECUTE
// (which doesn't exist generically) and redefines PKEY_ACCESS_MASK to
// include it -- but, unlike aarch64, powerpc does NOT add a
// PKEY_DISABLE_READ bit at all. Compare x86_64/arm (PKEY_ACCESS_MASK = 0x3)
// and aarch64 (PKEY_ACCESS_MASK = 0xf, includes DISABLE_READ).
...
/// Disable execute access for memory tagged with this protection key
/// (powerpc-specific override; no generic equivalent).
pub const PKEY_DISABLE_EXECUTE: u32 = 0x4; // powerpc-specific override
...
pub const PKEY_ACCESS_MASK: u32 = PKEY_DISABLE_ACCESS | PKEY_DISABLE_WRITE |
    PKEY_DISABLE_EXECUTE; // = 0x7, powerpc-specific override
