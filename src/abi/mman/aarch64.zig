//! Linux mmap()/mprotect()/mlock()/madvise() ABI constants for aarch64.
//!
...
//!     LOCKED/NORESERVE, MCL_*, SHADOW_STACK_SET_*) -- arm64 includes this
//!     file unmodified via `arch/arm64/include/uapi/asm/mman.h`
//!   - `arch/arm64/include/uapi/asm/mman.h` (adds PROT_BTI, PROT_MTE; then
//!     OVERRIDES PKEY_DISABLE_EXECUTE, PKEY_DISABLE_READ, and redefines
//!     PKEY_ACCESS_MASK to include both -- see PKEY_* section below)
...
//!
//! Flattened: arm64 pulls in the generic mman.h chain unmodified for
//! MAP_*/MCL_*, but adds two PROT_* bits and overrides part of the PKEY_*
//! group. Contrast with x86_64/arm (no PKEY_* override) and powerpc (much
//! larger divergence, documented in that file's header).
//!
...
const log = std.log.scoped(.abi_mman_aarch64);
...
// arm64-specific: these occupy the two bits the generic header reserves for
// arch-specific use (0x10, 0x20) -- not present on x86_64 or arm.
/// arm64-only: marks the page as a Branch Target Identification (BTI)
/// guarded landing pad for indirect branches.
pub const PROT_BTI: u32 = 0x10; // BTI guarded page
/// arm64-only: enables Memory Tagging Extension (MTE) tag checking for
/// this normal (non-device) mapping.
...
//
// arm64 OVERRIDES part of the generic set: it adds PKEY_DISABLE_EXECUTE and
// PKEY_DISABLE_READ (which don't exist generically), and redefines
// PKEY_ACCESS_MASK to combine all four DISABLE_* bits instead of just the
// generic two. Compare x86_64/arm (PKEY_ACCESS_MASK = 0x3, no EXECUTE/READ
// bits) and powerpc (its own different override, no DISABLE_READ at all).
...
/// arm64-only: disable execute access for memory tagged with this
/// protection key.
pub const PKEY_DISABLE_EXECUTE: u32 = 0x4; // arm64-specific override
/// arm64-only: disable read access for memory tagged with this protection
/// key.
pub const PKEY_DISABLE_READ: u32 = 0x8; // arm64-specific override
...
pub const PKEY_ACCESS_MASK: u32 = PKEY_DISABLE_ACCESS | PKEY_DISABLE_WRITE |
    PKEY_DISABLE_READ | PKEY_DISABLE_EXECUTE; // = 0xf, arm64-specific override
