//! Linux mmap()/mprotect()/mlock()/madvise() ABI constants for x86_64.
//!
...
//!     LOCKED/NORESERVE, MCL_*, SHADOW_STACK_SET_*) -- x86 includes this file
//!     unmodified via `arch/x86/include/uapi/asm/mman.h`
//!   - `arch/x86/include/uapi/asm/mman.h` (adds MAP_32BIT, MAP_ABOVE4G on top
//!     of the asm-generic/mman.h include)
...
//!
//! Flattened: x86_64 pulls in the generic mman.h chain unmodified and adds
//! two extras, so this file is just "generic + MAP_32BIT + MAP_ABOVE4G".
//! Contrast with `powerpc.zig`, which diverges from generic in several
//! places (see that file's header for details).
//!
...
const log = std.log.scoped(.abi_mman_x86_64);
...
// header; x86_64 does not define anything there (contrast aarch64's
...
// x86_64-specific: only defined here, not part of the generic base.
/// x86_64-only: restrict the mapping to the low 2GB of address space so
/// pointers into it fit in 32 bits.
