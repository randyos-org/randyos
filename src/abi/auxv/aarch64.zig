//! Linux auxiliary vector (`AT_*`) type constants for aarch64.
//!
//! Sourced from the Linux kernel source tree (`include/uapi/linux/auxvec.h`
//! and `arch/arm64/include/uapi/asm/auxvec.h`), torvalds/linux @
...
//!
//! `arch/arm64/include/uapi/asm/auxvec.h` redundantly re-declares
//! `AT_MINSIGSTKSZ = 51` alongside its own `AT_SYSINFO_EHDR = 33` (the
//! generic header guards its own copy with `#ifndef AT_MINSIGSTKSZ`);
...
pub const Type = enum(u32) {
    ...
    // own AT_*CACHEBSIZE/AT_IGNOREPPC extras; not applicable to aarch64).
    ...
    /// arm64-specific extra (`arch/arm64/include/uapi/asm/auxvec.h`): address
    /// of the vDSO ELF image mapped into the process.
    AT_SYSINFO_EHDR = 33,
