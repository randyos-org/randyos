//! Linux auxiliary vector (`AT_*`) type constants for arm (32-bit).
//!
//! Sourced from the Linux kernel source tree (`include/uapi/linux/auxvec.h`
//! and `arch/arm/include/uapi/asm/auxvec.h`), torvalds/linux @
...
const log = std.log.scoped(.abi_auxv_arm);
...
pub const Type = enum(u32) {
    ...
    // own AT_*CACHEBSIZE/AT_IGNOREPPC extras; not applicable to arm).
    ...
    /// arm-specific extra (`arch/arm/include/uapi/asm/auxvec.h`): address of
    /// the vDSO ELF image mapped into the process.
    AT_SYSINFO_EHDR = 33,
