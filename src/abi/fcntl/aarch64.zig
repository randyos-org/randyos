//! Linux `open()`/`fcntl()` flag and command constants for aarch64.
//!
//! Sourced from the Linux kernel source tree
//! (`include/uapi/asm-generic/fcntl.h` plus the aarch64 override,
//! `arch/arm64/include/uapi/asm/fcntl.h`), torvalds/linux @
...
//!
//! `arch/arm64/include/uapi/asm/fcntl.h` DOES exist and reorders four flags
//! relative to the generic/x86_64 positions ("Using our own definitions for
//! AArch32 (compat) support", per that file's comment):
//! O_DIRECTORY = (1 << 14), O_NOFOLLOW = (1 << 15), O_DIRECT = (1 << 16),
//! O_LARGEFILE = (1 << 17). Compare against x86_64.zig (plain generic
//! positions: O_DIRECT=14, O_LARGEFILE=15, O_DIRECTORY=16, O_NOFOLLOW=17)
//! and powerpc.zig (yet another ordering). arm.zig (32-bit) happens to use
//! this exact same override, bit-for-bit.
//!
//! F_GETLK64 / F_SETLK64 / F_SETLKW64 are guarded in the generic header by
//! `#if __BITS_PER_LONG == 32 || defined(__KERNEL__)`. aarch64 userspace
//! resolves __BITS_PER_LONG to 64 (see arch/arm64/include/uapi/asm/bitsperlong.h:
//! only `defined(__KERNEL__) && !defined(__aarch64__)` selects 32, which
//! never applies to native aarch64 userspace), so this uapi header does not
//! define these three commands for aarch64 -- they are omitted below. They
//! ARE present in arm.zig and powerpc.zig (32-bit __BITS_PER_LONG).
//!
...
const log = std.log.scoped(.abi_fcntl_aarch64);
...
// -- see file header. aarch64's arch/arm64/include/uapi/asm/fcntl.h
// override reorders these relative to the generic (x86_64) positions.
/// Hint to perform I/O directly between user-space buffers and the
/// underlying device, minimizing page-cache effects.
pub const O_DIRECT: u32 = 1 << 16;
...
pub const O_LARGEFILE: u32 = 1 << 17;
...
// F_GETLK64/F_SETLK64/F_SETLKW64 intentionally omitted -- see file header
// (__BITS_PER_LONG == 64 on aarch64 userspace excludes them from this uapi
// header).
