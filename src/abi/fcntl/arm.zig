//! Linux `open()`/`fcntl()` flag and command constants for arm (32-bit
//! EABI).
//!
//! Sourced from the Linux kernel source tree
//! (`include/uapi/asm-generic/fcntl.h` plus the arm override,
//! `arch/arm/include/uapi/asm/fcntl.h`), torvalds/linux @
...
//!
//! `arch/arm/include/uapi/asm/fcntl.h` DOES exist and reorders four flags
//! relative to the generic/x86_64 positions: O_DIRECTORY = (1 << 14),
//! O_NOFOLLOW = (1 << 15), O_DIRECT = (1 << 16) ("currently ignored", per
//! that file's comment), O_LARGEFILE = (1 << 17). Compare against
//! x86_64.zig (plain generic positions: O_DIRECT=14, O_LARGEFILE=15,
//! O_DIRECTORY=16, O_NOFOLLOW=17) and powerpc.zig (yet another ordering).
//! This is bit-for-bit the same override aarch64.zig uses.
//!
//! F_GETLK64 / F_SETLK64 / F_SETLKW64 are guarded in the generic header by
//! `#if __BITS_PER_LONG == 32 || defined(__KERNEL__)`. arm has no
//! arch-specific uapi/asm/bitsperlong.h override (that path 404s at the
//! pinned commit), so it falls back to
//! include/uapi/asm-generic/bitsperlong.h, which resolves __BITS_PER_LONG
//! to `__CHAR_BIT__ * __SIZEOF_LONG__` = 32 on this 32-bit arch -- so these
//! three commands ARE part of the arm uapi and are included below. Compare
//! x86_64.zig and aarch64.zig, where the 64-bit __BITS_PER_LONG excludes
//! them.
//!
...
const log = std.log.scoped(.abi_fcntl_arm);
...
// -- see file header. arm's arch/arm/include/uapi/asm/fcntl.h override
// reorders these relative to the generic (x86_64) positions -- identical
// to aarch64's override.
...
// F_GETLK64/F_SETLK64/F_SETLKW64 are part of the arm uapi -- see file
// header (__BITS_PER_LONG == 32 on arm userspace keeps them visible).
