//! Linux `open()`/`fcntl()` flag and command constants for x86_64.
//!
//! Sourced from the Linux kernel source tree
//! (`include/uapi/asm-generic/fcntl.h`), torvalds/linux @
//! 8cdeaa50eae8dad34885515f62559ee83e7e8dda (kernel version 7.2.0-rc2), by
//! fetching that file directly and mechanically extracting (name, value)
//! pairs -- not transcribed by hand. Re-derive from that same file if this
//! ever looks stale; do not hand-edit values here.
//!
//! `arch/x86/include/uapi/asm/fcntl.h` does not exist (confirmed: that path
//! 404s at the pinned commit) -- x86_64 has no per-arch override and simply
//! uses the generic header's bit positions for the four flags every other
//! arch's override header reorders: O_DIRECT = (1 << 14),
//! O_LARGEFILE = (1 << 15), O_DIRECTORY = (1 << 16), O_NOFOLLOW = (1 << 17).
//! Compare against aarch64.zig, arm.zig, and powerpc.zig, which each move
//! these four to different bit positions via their own arch/*/asm/fcntl.h.
//!
//! F_GETLK64 / F_SETLK64 / F_SETLKW64 are guarded in the generic header by
//! `#if __BITS_PER_LONG == 32 || defined(__KERNEL__)`. x86_64 userspace
//! resolves __BITS_PER_LONG to 64 (see arch/x86/include/uapi/asm/bitsperlong.h:
//! `defined(__x86_64__) && !defined(__ILP32__)` selects 64), so this uapi
//! header does not define these three commands for x86_64 -- they are
//! omitted below. They ARE present in arm.zig and powerpc.zig (32-bit
//! __BITS_PER_LONG).
//!
...
// -- see file header. x86_64 (this file) has no arch override, so it uses
// the plain generic asm-generic/fcntl.h positions.
...
// (__BITS_PER_LONG == 64 on x86_64 userspace excludes them from this uapi
// header).
